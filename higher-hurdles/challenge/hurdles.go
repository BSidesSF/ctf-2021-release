...
const FLAG = "CTF{good_work_on_hurdling_past_2020}"
...
func stringSliceContains(choices []string, needle string) bool {
...
func addAllHurdles() {
	addHurdle(
		...
		func(r *http.Request) error {
			v := r.URL.Query()
			if v.Get("retrieve") != "flag" {
				return fmt.Errorf("Your URL did not ask to `retrieve` the `flag` in its query string.")
			}
			return nil
		},
		func(r *http.Request) error {
		...
		func(r *http.Request) error {
			u, p, ok := r.BasicAuth()
			if !ok || u != "username" {
				return fmt.Errorf("Basically, I was expecting the username username.")
			}
			if p != "4ef03423738a4aa7956528feebbc65474c053f5937032dfb9219af62" {
				return fmt.Errorf("Basically, I was expecting the password to be the hex representation of the sha3-224 of the author of this challenge.")
			}
			return nil
		},
		func(r *http.Request) error {
		...
		func(r *http.Request) error {
			...
			cli_ip := "13.37.37.13"
			for _, p := range []string{"127.1.1.1", "10.5.4.3", "19.18.0.1"} {
				if !stringSliceContains(s[1:], p) {
					return fmt.Errorf("I was expecting this to be forwarded through %s", p)
				}
			}
			if s[0] != cli_ip {
		...
		func(r *http.Request) error {
			...
			if !strings.HasPrefix(r.Header.Get("Accept-Language"), lang) {
				return fmt.Errorf("Ich hatte erwartet, dass Sie Deutsch akzeptieren.")
			}
			return nil
		},
		func(r *http.Request) error {
		...
		func(r *http.Request) error {
			if r.Header.Get("DNT") == "" {
				return fmt.Errorf("Surely you'd like to express your opinion on tracking on the web as a header?")
			}
			return nil
		},
		func(r *http.Request) error {
			...
			if fetchSite != "same-site" && fetchSite != "same-origin" {
				return fmt.Errorf("I expected fetch metadata from the same site or origin.")
			}
			return nil
		},
		func(r *http.Request) error {
			if r.Header.Get("Sec-Fetch-Mode") != "navigate" {
				return fmt.Errorf("I expected the mode of this fetch to be a navigate.")
			}
			return nil
		},
		func(r *http.Request) error {
			if r.Header.Get("Sec-Fetch-User") != "?1" {
				return fmt.Errorf("I expected this fetch to be user activated.")
			}
			return nil
		},
	)
